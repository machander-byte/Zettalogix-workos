...
export default function EmployeeCallsPage() {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      setLoading(true);
      try {
        const data = await callService.logsMe();
        setLogs(data);
      } finally {
        setLoading(false);
      }
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/90 shadow-sm">
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && withCounterpart.length === 0 && (
              <tr>
        ...
        {loading && (
          <p className="px-6 py-4 text-sm text-slate-500">Loading call history...</p>
        )}
      </div>
